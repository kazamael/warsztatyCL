document.addEventListener('DOMContentLoaded', function () {
    ...
    input2.addEventListener("input", function () {
        ...
        if (input2.value < 0 || input2.value.isNaN) {
            input2.value="tylko liczby >=0";
        } else {
    ...
    const openShop = document.querySelector(".section__button");

    openShop.addEventListener("click", function () {
        document.querySelector(".calc-box").classList.remove("display-none");
    })

    document.querySelector(".total").addEventListener("click", function () {
        document.querySelector(".calc-box").classList.add("display-none");
    })

    const bundleNaw=document.querySelectorAll(".product__button");

    for(let i=0;i<bundleNaw.length;i++){
        bundleNaw[i].addEventListener("click", function () {
            document.querySelector(".calc-box").classList.remove("display-none");
            const product = document.getElementsByClassName("result-item-package")[0];
            product.classList.remove("display-none");
            product.querySelector("li:nth-of-type(3)").innerHTML = "$" + this.dataset.price;
